using System.Configuration;
using System.Diagnostics.Contracts;

namespace MqttCore.Report
{
    public sealed class StimulSoftElement : ConfigurationSection
    {
        [ConfigurationProperty(name: "Name", IsKey = true)]
        public string Name
        {
            get
            {
                return this["Name"].ToString();
            }
            set
            {
                this["Name"] = value;
            }
        }

        [ConfigurationProperty(name: "Licence", DefaultValue = "")]
        public string Licence
        {
            get
            {
                return this["Licence"].ToString();
            }
            set
            {
                this["Licence"] = value;
            }
        }

        [ConfigurationProperty(name: "FilePath", DefaultValue = @"Templates\Reports", IsRequired = true)]
        public string FilePath
        {
            get
            {
                return this["FilePath"].ToString();
            }
            set
            {
                this["FilePath"] = value;
            }
        }

        [ConfigurationProperty(name: "FileName", IsRequired = true)]
        public string FileName
        {
            get
            {
                return this["FileName"].ToString();
            }
            set
            {
                this["FileName"] = value;
            }
        }

        public StimulSoftElement()
        {
        }
    }
}